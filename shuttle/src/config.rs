...
#[cfg(windows)]
pub const DEFAULT_IPC_ENDPOINT: &str = r"\\.\pipe\shuttle.ipc";

/// The default IPC endpoint
#[cfg(not(windows))]
pub const DEFAULT_IPC_ENDPOINT: &str = "/tmp/shuttle.ipc";

/// `shuttle 0.1.0 (f01b232bc 2022-04-13T23:28:39.493201+00:00)`
...
pub struct NodeConfig {
    ...
    /// Filename to write shuttle output as json
        ...
        if self.config_out.is_some() {
            ...
            to_writer(
                &File::create(config_out)
                    .expect("Unable to create shuttle config description file"),
                &self.as_json(fork),
            )
...
impl AccountGenerator {
    pub fn gen(&self) -> Vec<Wallet<SigningKey>> {
        ...
        for i in 1..=self.amount {
            ...
            priv_key[56] = i as u8;
            let wallet =
                Wallet::from_bytes(&priv_key, corebc::types::Network::from(self.chain_id)).unwrap();
            wallets.push(wallet);
        }
