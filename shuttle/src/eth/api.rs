...
/// The client version: `shuttle/v{major}.{minor}.{patch}`
pub const CLIENT_VERSION: &str = concat!("shuttle/v", env!("CARGO_PKG_VERSION"));
    ...
    pub async fn execute(&self, request: EthRequest) -> ResponseResult {
        ...
        match request {
            ...
            EthRequest::ImpersonateAccount(addr) => {
                self.shuttle_impersonate_account(addr).await.to_rpc_result()
            }
            EthRequest::StopImpersonatingAccount(addr) => {
                self.shuttle_stop_impersonating_account(addr).await.to_rpc_result()
            }
            EthRequest::AutoImpersonateAccount(enable) => {
                self.shuttle_auto_impersonate_account(enable).await.to_rpc_result()
            }
            EthRequest::GetAutoMine(()) => self.shuttle_get_auto_mine().to_rpc_result(),
            EthRequest::Mine(blocks, interval) => {
                self.shuttle_mine(blocks, interval).await.to_rpc_result()
            }
            EthRequest::SetAutomine(enabled) => {
                self.shuttle_set_auto_mine(enabled).await.to_rpc_result()
            }
            EthRequest::SetIntervalMining(interval) => {
                self.shuttle_set_interval_mining(interval).to_rpc_result()
            }
            EthRequest::DropTransaction(tx) => {
                self.shuttle_drop_transaction(tx).await.to_rpc_result()
            }
            EthRequest::Reset(fork) => {
                self.shuttle_reset(fork.and_then(|p| p.params)).await.to_rpc_result()
            }
            EthRequest::SetBalance(addr, val) => {
                self.shuttle_set_balance(addr, val).await.to_rpc_result()
            }
            EthRequest::SetCode(addr, code) => {
                self.shuttle_set_code(addr, code).await.to_rpc_result()
            }
            EthRequest::SetNonce(addr, nonce) => {
                self.shuttle_set_nonce(addr, nonce).await.to_rpc_result()
            }
            EthRequest::SetStorageAt(addr, slot, val) => {
                self.shuttle_set_storage_at(addr, slot, val).await.to_rpc_result()
            }
            EthRequest::SetCoinbase(addr) => self.shuttle_set_coinbase(addr).await.to_rpc_result(),
            EthRequest::SetLogging(log) => self.shuttle_set_logging(log).await.to_rpc_result(),
            EthRequest::SetMinGasPrice(energy) => {
                self.shuttle_set_min_energy_price(energy).await.to_rpc_result()
            }
            EthRequest::DumpState(_) => self.shuttle_dump_state().await.to_rpc_result(),
            EthRequest::LoadState(buf) => self.shuttle_load_state(buf).await.to_rpc_result(),
            EthRequest::NodeInfo(_) => self.shuttle_node_info().await.to_rpc_result(),
            ...
            EthRequest::SetRpcUrl(url) => self.shuttle_set_rpc_url(url).to_rpc_result(),
            EthRequest::EthSendUnsignedTransaction(tx) => {
                self.eth_send_unsigned_transaction(*tx).await.to_rpc_result()
            }
            EthRequest::EnableTraces(_) => self.shuttle_enable_traces().await.to_rpc_result(),
...
// == impl EthApi shuttle endpoints ==

impl EthApi {
    /// Send transactions impersonating specific account and contract addresses.
    ///
    /// Handler for ETH RPC call: `shuttle_impersonateAccount`
    pub async fn shuttle_impersonate_account(&self, address: Address) -> Result<()> {
        node_info!("shuttle_impersonateAccount");
    ...
    ///
    /// Handler for ETH RPC call: `shuttle_stopImpersonatingAccount`
    pub async fn shuttle_stop_impersonating_account(&self, address: Address) -> Result<()> {
        node_info!("shuttle_stopImpersonatingAccount");
    ...
    ///
    /// Handler for ETH RPC call: `shuttle_autoImpersonateAccount`
    pub async fn shuttle_auto_impersonate_account(&self, enabled: bool) -> Result<()> {
        node_info!("shuttle_autoImpersonateAccount");
    ...
    ///
    /// Handler for ETH RPC call: `shuttle_getAutomine`
    pub fn shuttle_get_auto_mine(&self) -> Result<bool> {
        node_info!("shuttle_getAutomine");
    ...
    pub async fn shuttle_set_auto_mine(&self, enable_automine: bool) -> Result<()> {
    ...
    ///
    /// Handler for ETH RPC call: `shuttle_mine`
    pub async fn shuttle_mine(
        &self,
        num_blocks: Option<U256>,
        interval: Option<U256>,
    ) -> Result<()> {
        node_info!("shuttle_mine");
    ...
    pub fn shuttle_set_interval_mining(&self, secs: u64) -> Result<()> {
    ...
    ///
    /// Handler for RPC call: `shuttle_dropTransaction`
    pub async fn shuttle_drop_transaction(&self, tx_hash: H256) -> Result<Option<H256>> {
        node_info!("shuttle_dropTransaction");
    ...
    ///
    /// Handler for RPC call: `shuttle_reset`
    pub async fn shuttle_reset(&self, forking: Option<Forking>) -> Result<()> {
        node_info!("shuttle_reset");
    ...
    ///
    /// Handler for RPC call: `shuttle_setBalance`
    pub async fn shuttle_set_balance(&self, address: Address, balance: U256) -> Result<()> {
        node_info!("shuttle_setBalance");
    ...
    ///
    /// Handler for RPC call: `shuttle_setCode`
    pub async fn shuttle_set_code(&self, address: Address, code: Bytes) -> Result<()> {
        node_info!("shuttle_setCode");
    ...
    ///
    /// Handler for RPC call: `shuttle_setNonce`
    pub async fn shuttle_set_nonce(&self, address: Address, nonce: U256) -> Result<()> {
        node_info!("shuttle_setNonce");
    ...
    ///
    /// Handler for RPC call: `shuttle_setStorageAt`
    pub async fn shuttle_set_storage_at(
        &self,
        address: Address,
        slot: U256,
        val: H256,
    ) -> Result<bool> {
        node_info!("shuttle_setStorageAt");
    ...
    ///
    /// Handler for RPC call: `shuttle_setLoggingEnabled`
    pub async fn shuttle_set_logging(&self, enable: bool) -> Result<()> {
        node_info!("shuttle_setLoggingEnabled");
    ...
    ///
    /// Handler for RPC call: `shuttle_setMinGasPrice`
    pub async fn shuttle_set_min_energy_price(&self, energy: U256) -> Result<()> {
        node_info!("shuttle_setMinGasPrice");
    ...
    ///
    /// Handler for RPC call: `shuttle_setCoinbase`
    pub async fn shuttle_set_coinbase(&self, address: Address) -> Result<()> {
        node_info!("shuttle_setCoinbase");
        self.backend.set_coinbase(address);
        Ok(())
    }

    /// Create a bufer that represents all state on the chain, which can be loaded to separate
    /// process by calling `shuttle_loadState`
    ///
    /// Handler for RPC call: `shuttle_dumpState`
    pub async fn shuttle_dump_state(&self) -> Result<Bytes> {
        node_info!("shuttle_dumpState");
    ...
    /// storage.
    ///
    /// Handler for RPC call: `shuttle_loadState`
    pub async fn shuttle_load_state(&self, buf: Bytes) -> Result<bool> {
        node_info!("shuttle_loadState");
        self.backend.load_state(buf).await
    }

    /// Retrieves the Anvil node configuration params.
    ///
    /// Handler for RPC call: `shuttle_nodeInfo`
    pub async fn shuttle_node_info(&self) -> Result<NodeInfo> {
        node_info!("shuttle_nodeInfo");
    ...
    ///
    /// Handler for RPC call: `shuttle_setBlockTimestampInterval`
    pub fn evm_set_block_timestamp_interval(&self, seconds: u64) -> Result<()> {
        node_info!("shuttle_setBlockTimestampInterval");
    ...
    /// Sets an interval for the block timestamp
    ///
    /// Handler for RPC call: `shuttle_removeBlockTimestampInterval`
    pub fn evm_remove_block_timestamp_interval(&self) -> Result<bool> {
        node_info!("shuttle_removeBlockTimestampInterval");
    ...
    /// compatibility reasons, this is a separate call since `evm_mine` is not an shuttle original.
                ...
                for tx in block.transactions.iter_mut() {
                    if let Some(receipt) = self.backend.mined_transaction_receipt(tx.hash) {
                        ...
                        if let Some(_output) = receipt.out {
                            todo!("CORETODO: Handle this: shuttle/src/eth/api.rs");
    ...
    ///
    /// Handler for ETH RPC call: `shuttle_setBlock`
    pub fn shuttle_set_block(&self, block_number: U256) -> Result<()> {
        node_info!("shuttle_setBlock");
    ...
    ///
    /// Handler for ETH RPC call: `shuttle_setRpcUrl`
    pub fn shuttle_set_rpc_url(&self, url: String) -> Result<()> {
        node_info!("shuttle_setRpcUrl");
    ...
    ///
    /// Handler for ETH RPC call: `shuttle_enableTraces`
    pub async fn shuttle_enable_traces(&self) -> Result<()> {
        node_info!("shuttle_enableTraces");
        Err(BlockchainError::RpcUnimplemented)
    }
    ...
    pub async fn txpool_inspect(&self) -> Result<TxpoolInspect> {
        ...
        fn convert(tx: Arc<PoolTransaction>) -> TxpoolInspectSummary {
            ...
            TxpoolInspectSummary { to, value, energy, energy_price }
        }

        // Note: naming differs geth vs shuttle:
        //
...
impl EthApi {
    ...
    ///
    /// This will execute the [EthTransactionRequest] and find the best energy limit via binary
    /// search
        ...
        D: DatabaseRef<Error = DatabaseError>,
    {
        ...
        let mut highest_energy_limit =
            request.energy.unwrap_or(block_env.energy_limit.to_ethers_u256());
        ...
        let energy_limit =
            std::cmp::min(request.energy.unwrap_or(highest_energy_limit), highest_energy_limit);
        ...
        let mut mid_energy_limit =
            std::cmp::min(energy * 3, (highest_energy_limit + lowest_energy_limit) / 2);
        ...
        while (highest_energy_limit - lowest_energy_limit) > U256::one() {
            ...
            // Exceptional case: init used too much energy, we need to increase the energy limit and
            // try again
            if let Err(BlockchainError::InvalidTransaction(InvalidTransactionError::GasTooHigh)) =
                ethres
            {
            ...
            match ethres {
                Ok((exit, _, _energy, _)) => match exit {
                    // If the transaction succeeded, we can set a ceiling for the highest energy
                    // limit at the current midpoint, as spending any more
                    // energy would make no sense (as the TX would still
                    // succeed).
                    return_ok!() => {
...
/// Executes the requests again after an out of energy error to check if the error is energy related
/// or not
#[inline]
...
fn determine_base_energy_by_kind(request: EthTransactionRequest) -> U256 {
    match request.into_typed_request() {
        ...
        // Tighten the energy limit upwards if we don't know the transaction type to avoid
        // deployments failing.
        _ => MIN_CREATE_GAS,
    }
}
