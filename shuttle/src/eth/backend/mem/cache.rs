...
use foxar_evm::executor::backend::snapshot::StateSnapshot;
use std::{
    io,
    path::{Path, PathBuf},
};
...
impl DiskStateCache {
    ...
    pub fn write(&mut self, hash: H256, state: StateSnapshot) {
        self.with_cache_file(hash, |file| {
            tokio::task::spawn(async move {
                match foxar_common::fs::write_json_file(&file, &state) {
                    Ok(_) => {
    ...
    pub fn read(&mut self, hash: H256) -> Option<StateSnapshot> {
        self.with_cache_file(hash, |file| {
            match foxar_common::fs::read_json_file::<StateSnapshot>(&file) {
                Ok(state) => {
    ...
    pub fn remove(&mut self, hash: H256) {
        self.with_cache_file(hash, |file| {
            foxar_common::fs::remove_file(file).map_err(|err| {
...
impl Default for DiskStateCache {
    fn default() -> Self {
        DiskStateCache { temp_path: shuttle_tmp_dir(), temp_dir: None }
    }
}

/// Returns the temporary dir for the cached state
///
/// This will create a prefixed temp dir with `shuttle-state-06-11-2022-12-50`
fn build_tmp_dir(p: Option<&Path>) -> io::Result<TempDir> {
    let mut builder = tempfile::Builder::new();
    let now = chrono::offset::Utc::now();
    let prefix = now.format("shuttle-state-%d-%m-%Y-%H-%M").to_string();
