...
pub type State = foxar_evm::HashMap<Address, Account>;
...
impl Backend {
    ...
    pub async fn with_genesis(
        ...
        let blockchain = if let Some(ref fork) = fork {
            ...
            Blockchain::new(&env.read(), genesis.timestamp)
        };
        ...
        if let Some(fork) = self.get_fork() {
            ...
            // update all settings related to the forked block
            {
                ...
                env.block = BlockEnv {
                    ...
                    timestamp: fork_block.timestamp.to_ruint(),
                    energy_limit: fork_block.energy_limit.to_ruint(),
                    difficulty: fork_block.difficulty.to_ruint(),
                    // Keep previous `coinbase` and `basefee` value
                    coinbase: env.block.coinbase,
                };

                self.time.reset(env.block.timestamp.to_ethers_u256().as_u64());
    ...
    pub fn set_block_number(&self, number: U256) {
        let mut env = self.env.write();
        env.block.number = number.to_ruint();
    }
    ...
    pub async fn current_balance(&self, address: Address) -> DatabaseResult<U256> {
        Ok(self.get_account(address).await?.balance.to_ethers_u256())
    }
    ...
    pub fn energy_limit(&self) -> U256 {
        self.env().read().block.energy_limit.to_ethers_u256()
    }

    /// Sets the block energy limit
    pub fn set_energy_limit(&self, energy_limit: U256) {
        self.env().write().block.energy_limit = energy_limit.to_ruint();
    }

    /// Returns the current energy price
    pub fn energy_price(&self) -> U256 {
        self.fees.energy_price()
    }

    /// Sets the energy price
    pub fn set_energy_price(&self, price: U256) {
        self.fees.set_energy_price(price)
    }
    ...
    > {
        ...
        let state: hashbrown::HashMap<H176, Account> =
        ...
        Ok((exit_reason, out, energy_used, state, logs.unwrap_or_default()))
    }
        ...
        let (outcome, header, block_hash) = {
            ...
            for (info, receipt) in transactions.into_iter().zip(receipts) {
                // log some tx info
                {
                    ...
                    node_info!("    Gas used: {}", receipt.energy_used());
                    match info.exit {
                        return_ok!() => (),
                        ...
                        InstructionResult::OutOfEnergy => {
                            node_info!("    Error: ran out of energy");
                        }
                        reason => {
    ...
    pub async fn call(
        ...
        self.with_database_at(block_request, |state, block| {
            ...
            let (exit, out, energy, state) = match overrides {
            ...
            trace!(target: "backend", "call return {:?} out: {:?} energy {} on block {}", exit, out, energy, block_number);
            Ok((exit, out, energy, state))
        }).await?
    }
    ...
    fn build_call_env(
        ...
        let EthTransactionRequest { from, to, energy, value, data, nonce, .. } = request;

        let FeeDetails { energy_price } = fee_details;

        let energy_limit = energy.unwrap_or(block_env.energy_limit.to_ethers_u256());
        let mut env = self.env.read().clone();
        env.block = block_env;

        let energy_price = energy_price.unwrap_or_else(|| self.energy_price());
        ...
        env.tx = TxEnv {
            ...
            value: value.unwrap_or_default().to_ruint(),
        ...
        D: DatabaseRef<Error = DatabaseError>,
    {
        ...
        let state: hashbrown::HashMap<H176, Account> =
            state.into_iter().map(|kv| (kv.0.into(), kv.1)).collect();
        let (exit_reason, energy_used, out) = match result_and_state.result {
        ...
        inspector.print_logs();
        Ok((exit_reason, out, energy_used, state))
    }
    ...
    pub async fn call_with_tracing(
        ...
        self.with_database_at(block_request, |state, block| {
            ...
            let res = inspector.tracer.unwrap_or_default().traces.geth_trace(energy_used.into(), opts);
            trace!(target: "backend", "trace call return {:?} out: {:?} energy {} on block {}", exit_reason, out, energy_used, block_number);
            Ok(res)
        })
        .await?
    }
    ...
    fn mined_transactions_in_block(&self, block: &Block) -> Option<Vec<Transaction>> {
        ...
        for hash in block.transactions.iter().map(|tx| tx.hash()) {
            ...
            let tx = transaction_build(Some(hash), tx, Some(block), Some(info));
        ...
        F: FnOnce(Box<dyn MaybeHashDatabase + '_>, BlockEnv) -> T,
    {
            ...
            Some(BlockRequest::Pending(pool_transactions)) => {
                let result = self
                    .with_pending_block(pool_transactions, |state, block| {
                        ...
                        let block = BlockEnv {
                            number: block.header.number.to_ruint(),
                            coinbase: block.header.beneficiary.into(),
                            timestamp: rU256::from(block.header.timestamp),
                            difficulty: block.header.difficulty.to_ruint(),
                            energy_limit: block.header.energy_limit.to_ruint(),
                        };
                        f(state, block)
                    })
                    .await;
        ...
        if block_number < self.env.read().block.number.to_ethers_u256() {
            {
            ...
            if let Some(fork) = self.get_fork() {
                if block_number == U256::from(fork.block_number()) {
                    ...
                    block.number = block_number.to_ruint();
        ...
        D: DatabaseRef<Error = DatabaseError>,
    {
        ...
        Ok(state.basic(address.into())?.unwrap_or_default().balance.to_ethers_u256())
    }
    ...
    pub async fn debug_trace_transaction(
        ...
        hash: H256,
        opts: GoCoreDebugTracingOptions,
    ) -> Result<GoCoreTrace, BlockchainError> {
        if let Some(traces) = self.mined_geth_trace_transaction(hash, opts.clone()) {
            return Ok(GoCoreTrace::Known(GoCoreTraceFrame::Default(traces)));
        }
        ...
        Ok(GoCoreTrace::Known(GoCoreTraceFrame::Default(Default::default())))
    }
    ...
    pub(crate) fn mined_transaction_receipt(&self, hash: H256) -> Option<MinedTransactionReceipt> {
        ...
        let EIP658Receipt { status_code, energy_used, logs_bloom, logs } = receipt.into();

        let index = info.transaction_index as usize;

        let block = self.blockchain.storage.read().blocks.get(&block_hash).cloned()?;

        // TODO store cumulative energy used in receipt instead
        ...
        // cumulative_energy_used = cumulative_energy_used.saturating_sub(energy_used);
    ...
    fn mined_transaction_by_block_hash_and_index(
        ...
        Some(transaction_build(Some(info.transaction_hash), tx, Some(&block), Some(info)))
    }
    ...
    pub async fn prove_account_at(
        ...
        let account_key = H256::from(sha3(address.as_bytes()));
        ...
        self.with_database_at(block_request, |block_db, _| {
            ...
            let account_proof = AccountProof {
                ...
                storage_proof: values
                    .into_iter()
                    .map(|storage_key| {
                        let key = H256::from(sha3(storage_key));
    ...
    fn validate_pool_transaction_for(
        ...
        if chain_id != network_id.into() {
            if let Some(legacy) = tx.as_legacy() {
                ...
                warn!(target: "backend", ?chain_id, ?network_id, "invalid chain id");
                return Err(InvalidTransactionError::InvalidChainId);
            }
        }

        if tx.energy_limit() < MIN_TRANSACTION_GAS {
            warn!(target: "backend", "[{:?}] energy too low", tx.hash());
            return Err(InvalidTransactionError::GasTooLow);
        }

        if tx.energy_limit() > env.block.energy_limit.to_ethers_u256() {
            warn!(target: "backend", "[{:?}] energy too high", tx.hash());
        ...
        let value = tx.value();
        // check sufficient funds: `energy * price + value`
        ...
        if account.balance < req_funds.to_ruint() {
...
pub fn transaction_build(
    ...
    transaction.block_hash =
        block.as_ref().map(|block| H256::from(sha3(&rlp::encode(&block.header))));
