//! tests for shuttle specific logic
...
async fn test_can_change_mining_mode() {
    ...
    assert!(api.shuttle_get_auto_mine().unwrap());

    let num = provider.get_block_number().await.unwrap();
    assert_eq!(num.as_u64(), 0);

    api.shuttle_set_interval_mining(1).unwrap();
    assert!(!api.shuttle_get_auto_mine().unwrap());
    ...
    api.shuttle_set_interval_mining(0).unwrap();
    assert!(!api.shuttle_get_auto_mine().unwrap());
