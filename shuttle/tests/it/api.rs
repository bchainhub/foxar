...
use shuttle_core::eth::{state::AccountOverride, transaction::EthTransactionRequest};
...
async fn can_call_on_pending_block() {
    ...
    for i in 1..10 {
        api.shuttle_set_coinbase(accounts[i % accounts.len()]).await.unwrap();
...
async fn can_call_with_state_override() {
    ...
    api.shuttle_set_auto_mine(true).await.unwrap();
