    ...
    match interpreter.contract.bytecode.bytecode()[interpreter.program_counter()] {
        opcode::SHA3 => {
            if interpreter.stack.peek(1) == Ok(revm::primitives::U256::from(0x40)) {
                ...
                let result = U256::from(sha3(interpreter.memory.get_slice(offset, 0x40)));

                mapping_slots
                    .entry(b176_to_h176(address))
                    .or_default()
                    .seen_sha3
        ...
        opcode::SSTORE => {
            if let Some(mapping_slots) =
                mapping_slots.get_mut(&b176_to_h176(interpreter.contract.address))
            {
