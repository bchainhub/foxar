            ...
            .run_async(|| {
                async {
                    let url = format!(
                        ...
                        args.id,
                        u64::from(args.network.unwrap_or_default()),
                    );
...
metadata output can be enabled via `extra_output = ["metadata"]` in `foxar.toml`"#,
                args.contract.name
            )
        }
...
metadata output can be enabled via `extra_output = ["metadata"]` in `foxar.toml`"#,
        ...
        let req = SourcifyVerifyRequest {
            ...
            chain: u64::from(args.network.unwrap_or_default()).to_string(),
            files,
