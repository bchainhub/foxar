...
use crate::cmd::{
    get_cached_entry_by_name, read_constructor_args_file,
    spark::verify::provider::VerificationProvider, LoadConfig,
};
use corebc::{
    abi::Function,
    blockindex::{
            ...
            .run_async(|| {
                async {
                    ...
                    .wrap_err_with(|| {
                        ...
                        error!(target : "spark::verify",  ?args, "Failed to submit verification");
                        format!("Failed to submit contract verification, payload:\n{args}")
                    })?;

                    trace!(target : "spark::verify",  ?resp, "Received verification response");
