...
pub struct ScriptArgs {
    ...
    #[clap(
        long,
        requires = "sender",
        conflicts_with_all = &["private_key", "private_keys", "froms"],
    )]
    pub unlocked: bool,
                ...
                bytecodes.iter().find(|(_, init_code, _)| *init_code == &data[offset..])
            {
                ...
                if deployment_size > CONTRACT_MAX_SIZE {
                    ...
                    shell::println(format!(
                        "{}",
                        Paint::red(format!(
                            "`{name}` is above the contract size limit ({deployment_size} > {CONTRACT_MAX_SIZE})."
                        ))
                    ))?;
                }
            }
...
mod tests {
    ...
    fn can_extract_script_rpc_alias() {
        ...
        let config = r#"
                ...
                [rpc_endpoints]
                devin = "https://devin.com/v2/${_CAN_EXTRACT_RPC_ALIAS}"
            "#;
