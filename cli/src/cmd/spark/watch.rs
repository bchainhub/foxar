...
/// Executes a [`Watchexec`] that listens for changes in the project's src dir and reruns `spark
/// build`
...
struct WatchTestState {
    ...
    /// introduced by `spark fmt`
    last_test_files: HashSet<String>,
}

/// The `on_action` hook for `spark test --watch`
...
fn cmd_args(num: usize) -> Vec<String> {
    // all the spark arguments including path to spark bin
